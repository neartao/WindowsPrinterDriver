...
LPCTSTR InstallWindow::GetWindowClassName() const
{
	return _T("InstallWindow");
}

void InstallWindow::OnFinalMessage(HWND hWnd)
{
	WindowImplBase::OnFinalMessage(hWnd);
	delete this;
}

tString InstallWindow::GetSkinFile()
{
...
tString InstallWindow::GetSkinFolder()
{
	return tString(CPaintManagerUI::GetInstancePath());
}

LONG InstallWindow::GetStyle()
{
...
LRESULT InstallWindow::OnSysCommand(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
...
LRESULT InstallWindow::HandleMessage(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	return __super::HandleMessage(uMsg, wParam, lParam);
}

LRESULT InstallWindow::ResponseDefaultKeyEvent(WPARAM wParam)
{
	if (wParam == VK_RETURN)
	{
		return FALSE;
...
void InstallWindow::OnTimer(TNotifyUI& msg)
{
}

void InstallWindow::OnExit(TNotifyUI& msg)
{
	Close();
}

void InstallWindow::Init()
{
...
void InstallWindow::OnPrepare(TNotifyUI& msg)
{
...
void InstallWindow::UpdateNavigateButtonsStatus()
{
...
void InstallWindow::Notify(TNotifyUI& msg)
{
		...
		else if (_tcsicmp(msg.pSender->GetName(), kNextButtonControlName) == 0)
		{
			...
			if (next_pages_ == 0 && !driver_install_started_)
			{
				...
				if ((pBackBtn != NULL) && (pNextBtn != NULL) && (pCloseBtn != NULL) && (pCancelBtn != NULL))
				{
					...
					while (!bEndLoop)
					{
						DWORD nRet = MsgWaitForMultipleObjectsEx(1, &install_thread_, 5 * 60 * 1000, QS_ALLINPUT, MWMO_INPUTAVAILABLE);
						switch (nRet)
						{
						case WAIT_OBJECT_0:
							bEndLoop = true;
...
void InstallWindow::PrinterInstalledFinish()
{
	CButtonUI* pBackBtn = static_cast<CButtonUI*>(paint_manager_.FindControl(kBackButtonControlName));
	CButtonUI* pNextBtn = static_cast<CButtonUI*>(paint_manager_.FindControl(kNextButtonControlName));
	CButtonUI* pCloseBtn = static_cast<CButtonUI*>(paint_manager_.FindControl(kCloseButtonControlName));
	...
	if ((pBackBtn != NULL) && (pNextBtn != NULL) && (pCloseBtn != NULL) && (pCancelBtn != NULL))
	{
		...
		if (!driver_installed_)
		{
			...
			DUIMessageBox(m_hWnd, error_info_.c_str(), _T("失败"));
		}
		else
		{
			AddProgressInfo(_T("添加打印机成功"), 100);
			DUIMessageBox(m_hWnd, _T("Mirco Unidrv打印机已添加成功！"), _T("Mirco Unidrv 驱动安装程序"));
			Close();
		}
	}
}

void InstallWindow::AddProgressInfo(const tString& info, int progress, bool red)
{
...
tString InstallWindow::GetDriverConfig(const TCHAR* name)
{
...
DWORD InstallWindow::InstallPrinterDriver(LPVOID pVoid)
{
	InstallWindow* pThis = (InstallWindow*)pVoid;
...
//
	...
	//{
	//	HKEY hSubkey2 = NULL;
	//	RegCreateKey(hSubkey, _T("PrinterDriverData"), &hSubkey2);
	//	if (hSubkey2 != NULL)
	//	{
	//		DWORD Val = 0x01;
	//		RegSetValueEx(hSubkey2, _T("UpsideDown"), 0, REG_DWORD, (BYTE*)&Val, sizeof(DWORD));

	//		Val = 0x01;
	//		RegSetValueEx(hSubkey2, _T("Cutter"), 0, REG_DWORD, (BYTE*)&Val, sizeof(DWORD));

	//		Val = 0x02;
	//		RegSetValueEx(hSubkey2, _T("CashDrawers"), 0, REG_DWORD, (BYTE*)&Val, sizeof(DWORD));

	//		Val = 0x01;
	//		RegSetValueEx(hSubkey2, _T("Buzzer"), 0, REG_DWORD, (BYTE*)&Val, sizeof(DWORD));

	//		Val = 0x01;
	//		RegSetValueEx(hSubkey2, _T("Barcode"), 0, REG_DWORD, (BYTE*)&Val, sizeof(DWORD));

	//		Val = 0x01;
	//		RegSetValueEx(hSubkey2, _T("Logo"), 0, REG_DWORD, (BYTE*)&Val, sizeof(DWORD));

	//		Val = 0x02;
	//		RegSetValueEx(hSubkey2, _T("PrinterType"), 0, REG_DWORD, (BYTE*)&Val, sizeof(DWORD));

	//		Val = 0x05;
	//		RegSetValueEx(hSubkey2, _T("CutterMode_default"), 0, REG_DWORD, (BYTE*)&Val, sizeof(DWORD));

	//		Val = 0x00320002;
	//		RegSetValueEx(hSubkey2, _T("DrawerMode1_default"), 0, REG_DWORD, (BYTE*)&Val, sizeof(DWORD));

	//		Val = 0x00320000;
	//		RegSetValueEx(hSubkey2, _T("DrawerMode2_default"), 0, REG_DWORD, (BYTE*)&Val, sizeof(DWORD));
	...
	TCHAR szOEMHelpFile[MAX_PATH] = {0};
	_stprintf_s(szOEMHelpFile, MAX_PATH - 1, pThis->GetDriverConfig(_T("OEMHelpFile")).c_str());
	temp = files_copied_path + szOEMHelpFile;
	temp2 = driver_dir_x86 + _T("3\\") + szOEMHelpFile;
	DeleteFile(temp2.c_str());
	temp2 = driver_dir_x86 + szOEMHelpFile;
	DeleteFile(temp2.c_str());
	CopyFile(temp.c_str(), temp2.c_str(), FALSE);
	...
	if (pBuf != NULL)
	{
		...
		pBased += _tcslen(pBased) + 1;
		_stprintf_s(pBased, MAX_PATH - 1, _T("%s"), szOEMHelpFile);
		pBased[_tcslen(pBased)] = _T('\0');
		...
		if (/*IsPrinterDriverInstalled(szDriverName) ||*/ AddPrinterDriver(NULL, 3, (LPBYTE)&drvinfo3))
		{
			...
			if (!IsPrinterAdded(print_name, print_name))
			{
				...
				{
					_stprintf_s(szPrinterName, MAX_PATH - 1, _T("SYSTEM\\CurrentControlSet\\Control\\Print\\Printers\\%s\\PrinterDriverData"), print_name.c_str());
					HKEY hKey = NULL;
					HKEY hSubkey = NULL;
					//Insert x86 additional drivers to registry
					RegOpenKeyEx(HKEY_LOCAL_MACHINE, szPrinterName, 0, KEY_ALL_ACCESS, &hKey);
					if (hKey != NULL)
					{
						DWORD Val = 0x01;
						RegSetValueEx(hKey, _T("UpsideDown"), 0, REG_DWORD, (BYTE*)&Val, sizeof(DWORD));

						Val = 0x01;
						RegSetValueEx(hKey, _T("Cutter"), 0, REG_DWORD, (BYTE*)&Val, sizeof(DWORD));

						Val = 0x02;
						RegSetValueEx(hKey, _T("CashDrawers"), 0, REG_DWORD, (BYTE*)&Val, sizeof(DWORD));

						Val = 0x01;
						RegSetValueEx(hKey, _T("Buzzer"), 0, REG_DWORD, (BYTE*)&Val, sizeof(DWORD));

						Val = 0x01;
						RegSetValueEx(hKey, _T("Barcode"), 0, REG_DWORD, (BYTE*)&Val, sizeof(DWORD));

						Val = 0x01;
						RegSetValueEx(hKey, _T("Logo"), 0, REG_DWORD, (BYTE*)&Val, sizeof(DWORD));

						Val = 0x02;
						RegSetValueEx(hKey, _T("PrinterType"), 0, REG_DWORD, (BYTE*)&Val, sizeof(DWORD));

						Val = 0x05;
						RegSetValueEx(hKey, _T("CutterMode_default"), 0, REG_DWORD, (BYTE*)&Val, sizeof(DWORD));

						Val = 0x00320002;
						RegSetValueEx(hKey, _T("DrawerMode1_default"), 0, REG_DWORD, (BYTE*)&Val, sizeof(DWORD));

						Val = 0x00320000;
						RegSetValueEx(hKey, _T("DrawerMode2_default"), 0, REG_DWORD, (BYTE*)&Val, sizeof(DWORD));
...
bool InstallWindow::IsPrinterDriverInstalled(const tString& driver_name)
{
...
bool InstallWindow::IsPrinterAdded(tString printer_name, tString& valid_name)
{
...
void InstallWindow::ConfigSerialPort()
{
	enum FlowControl
	{
		NoFlowControl,
